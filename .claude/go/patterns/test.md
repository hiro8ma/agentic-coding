...
- 2 回続けて実行しても通るようにする。前の実行が残したデータに依存しない
- DB のデータを消すときは、外部キーで参照している子テーブルから順に消す
- 固定時間の `time.Sleep` で待たず、条件がそろうまで待つ（チャネル、`sync.WaitGroup`、条件を満たすまでのポーリング）
- エミュレーター用の環境変数（接続先のホストなど）を設定し忘れると、実際の環境につながることがある。テストの前に確かめる
